...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <section className="section shell specialised-section"><div className="section-heading split-heading"><div><p className="eyebrow">Specialised cleaning</p><h2>Even The Small Things<br /><em>Deserve A Professional Clean.</em></h2></div><p>Discover dedicated cleaning for the everyday details around your home or workplace.</p></div><div className="specialised-grid">{specialised.map((service) => <Link href={`/services/${service.slug}`} className="special-pill" key={service.slug}><Image src={service.image} alt={`Urban Shine ${service.title} service`} fill sizes="(max-width: 700px) 46vw, 20vw" /><span>{service.title}</span><b>↗</b></Link>)}</div></section>
