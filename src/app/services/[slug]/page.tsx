...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section shell what-section"><div className="section-heading split-heading"><div><p className="eyebrow">Service focus</p><h2>What We <em>Clean</em></h2></div><p>These are editable examples of the accessible areas that may be included in your cleaning requirement.</p></div><div className="what-grid">{service.whatWeClean.map((area, index) => <article key={area} className="what-card"><Image src={service.whatWeCleanImages?.[index] || service.image} alt={`Urban Shine cleaning ${area}`} fill sizes="(max-width: 700px) 44vw, 16vw" /><div><span>0{index + 1}</span><h3>{area}</h3></div></article>)}</div></section>
    <section className="section process-section"><div className="shell"><div className="section-heading center-heading"><p className="eyebrow">A clear approach</p><h2>Our Cleaning <em>Process</em></h2><p>A simple, considered sequence from your shared requirement through to the final look.</p></div><div className="process-grid">{process.map(([title, copy, image], index) => <article className="process-card" key={title}><div className="process-image"><Image src={image} alt={`Cleaning process: ${title}`} fill sizes="(max-width: 700px) 90vw, 22vw" /><span>0{index + 1}</span></div><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
