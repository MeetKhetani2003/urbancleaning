...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    <section className="section shell what-section"><div className="section-heading split-heading"><div><p className="eyebrow">Service focus</p><h2>What We <em>Clean</em></h2></div><p>These are editable examples of the accessible areas that may be included in your cleaning requirement.</p></div><div className="what-grid">{service.whatWeClean.map((area, index) => <article key={area} className="what-card"><Image src={service.image} alt="" fill sizes="(max-width: 700px) 44vw, 16vw" /><div><span>0{index + 1}</span><h3>{area}</h3></div></article>)}</div></section>
