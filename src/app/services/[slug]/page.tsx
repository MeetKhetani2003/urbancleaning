...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!service) return {};
  return { title: `${service.title} Patna`, description: `${service.description} Request ${service.title.toLowerCase()} from Urban Shine Cleaning in Patna, Bihar.` };
}

const process = [
  ["Inspection", "We begin by understanding the cleaning areas and requirements you have shared.", "/images/how-it-works/choose-service.jpg"],
  ["Surface preparation", "The accessible space is prepared thoughtfully for the cleaning requirement.", "/images/how-it-works/schedule-service.jpg"],
  ["Deep cleaning", "The selected areas receive the focused professional cleaning attention requested.", "/images/how-it-works/professional-cleaning.jpg"],
  ["Final inspection", "A final look helps ensure the agreed focus areas have received attention.", "/images/how-it-works/spotless-home.jpg"],
];
...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <section className="service-hero"><Image src={service.image} alt={service.title} fill priority sizes="100vw" /><div className="service-hero-wash" /><div className="shell service-hero-content"><div><p className="eyebrow eyebrow--light">{service.category}</p><h1>Professional<br /><em>{service.title}</em></h1><p>{service.heroCopy}</p><Link href={`/book-service?service=${encodeURIComponent(service.title)}`} className="button button--light">Book {service.title} <span>↗</span></Link></div><div className="service-hero-mini"><span>Urban Shine</span><strong>Fresh attention<br />to every detail.</strong></div></div></section>
    <section className="section shell what-section"><div className="section-heading split-heading"><div><p className="eyebrow">Service focus</p><h2>What We <em>Clean</em></h2></div><p>These are editable examples of the accessible areas that may be included in your cleaning requirement.</p></div><div className="what-grid">{service.whatWeClean.map((area, index) => <article key={area} className="what-card"><Image src={`https://loremflickr.com/400/400/${encodeURIComponent(service.slug + ',' + area)}`} alt="" fill sizes="(max-width: 700px) 44vw, 16vw" /><div><span>0{index + 1}</span><h3>{area}</h3></div></article>)}</div></section>
    <section className="section process-section"><div className="shell"><div className="section-heading center-heading"><p className="eyebrow">A clear approach</p><h2>Our Cleaning <em>Process</em></h2><p>A simple, considered sequence from your shared requirement through to the final look.</p></div><div className="process-grid">{process.map(([title, copy, image], index) => <article className="process-card" key={title}><div className="process-image"><Image src={image} alt="" fill sizes="(max-width: 700px) 90vw, 22vw" /><span>0{index + 1}</span></div><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
